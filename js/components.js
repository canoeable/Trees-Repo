...
// data = a function returning the content (actually HTML)
	...
	Vue.component('upgrades', {
		...
		<div v-if="tmp[layer].upgrades" class="upgTable">
			<span v-for="row in (data === undefined ? tmp[layer].upgrades.rows : data)" :class="{upgRow: !(layer == 'c' && layer == 'd' && layer == 'h' && layer == 's')}">
	...
	Vue.component('upgrade', {
		props: ['layer', 'data'],
		template: `
		<button v-if="tmp[layer].upgrades && tmp[layer].upgrades[data]!== undefined && tmp[layer].upgrades[data].unlocked  && !(hasUpgrade(layer,data) && options.hideOwnedUpgs && (layer == 'c' || layer == 'd' || layer == 'h' || layer == 's'))" :id='"upgrade-" + layer + "-" + data' v-on:click="buyUpg(layer, data)" v-bind:class="{ [layer]: true, tooltipBox: true, upg: true, bought: hasUpgrade(layer, data), locked: (!(canAffordUpgrade(layer, data))&&!hasUpgrade(layer, data)), can: (canAffordUpgrade(layer, data)&&!hasUpgrade(layer, data))}"
	...
	Vue.component('prestige-button', {
		props: ['layer', 'data'],
		template: `
		<button v-if="(tmp[layer].type !== 'none')" v-bind:class="{ [layer]: true, reset: true, can: tmp[layer].canReset}"
	...
	Vue.component('microtabs', {
		...
		template: `
		<div v-if="tmp[layer].microtabs" :style="{'border-style': 'transparent'}">
			<div class="upgTable instant">
